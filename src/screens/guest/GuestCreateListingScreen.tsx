...
export default function GuestCreateListingScreen() {
  ...
  const handlePhotoSelect = async () => {
    ...
    if (remaining <= 0) { showToast('Max 4 photos', 'error'); return; }
    try {
      const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (status !== 'granted') { showToast('Photo permission required', 'error'); return; }
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        try {
          ...
          setPhotosPreviews(p => p.map((item, idx) => idx === targetIdx ? { ...item, uploading: false, cloudUrl } : item));
          setForm(p => ({ ...p, photos: [...p.photos, cloudUrl] }));
        } catch {
          setPhotosPreviews(p => p.map((item, idx) => idx === targetIdx ? { ...item, uploading: false, error: true } : item));
        }
      }
    } catch { showToast('Photo failed', 'error'); }
  };
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const payload: any = { ...form, expires_at: toLocalISO(form.expires_at) };
  ...
  const confirmExit = () => {
    if (form.title.trim() || form.quantity.trim()) {
      Alert.alert('Discard listing?', 'Your changes will be lost.', [
        ...
        { text: 'Discard', style: 'destructive', onPress: () => router.push('/guest/home' as any) },
      ]);
    } else {
      router.push('/guest/home' as any);
    }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.bg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          {STEPS.map((label, i) => {
            ...
            return (
              <React.Fragment key={s}>
                <TouchableOpacity onPress={() => { if (done) setStep(s); }} activeOpacity={done ? 0.7 : 1} style={{ alignItems: 'center', gap: 4 }}>
                  <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: done ? C.blue : active ? C.blue : C.surface2, alignItems: 'center', justifyContent: 'center', borderWidth: done || active ? 0 : 1.5, borderColor: C.border }}>
                    {done
                      ? <MaterialCommunityIcons name="check" size={14} color="#fff" />
                      : <Text style={{ fontSize: 12, fontWeight: '700', color: active ? '#fff' : C.textLight }}>{s}</Text>
                    }
                  </View>
                  <Text style={{ fontSize: 10, fontWeight: active ? '700' : '500', color: active ? C.blue : done ? C.textMid : C.textLight, width: 60, textAlign: 'center' }} numberOfLines={1}>{label}</Text>
                </TouchableOpacity>
                {i < STEPS.length - 1 && (
                  <View style={{ flex: 1, height: 2, borderRadius: 1, backgroundColor: done ? C.blue : C.border, marginBottom: 14 }} />
                )}
              </React.Fragment>
            );
          })}
        ...
        {step === 1 && (
          <View>
            ...
            <View style={{ backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.border, padding: 16, gap: 12 }}>
              <Input label="Title" value={form.title} onChangeText={set('title')} placeholder="e.g. Fresh dal bhat for 4 people" required accentColor={C.blue} />
              <Input label="Quantity" value={form.quantity} onChangeText={set('quantity')} placeholder="e.g. Serves 4 · 2 kg" required accentColor={C.blue} />
              <TextArea label="Description" value={form.description} onChange={set('description')} placeholder="Any details recipients should know…" rows={3} accentColor={C.blue} />
            </View>
          </View>
        )}
          ...
          <View>
            ...
            <View style={{ backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <Text style={{ fontSize: 12, color: form.tags.length > 0 ? C.blue : C.textLight }}>{form.tags.length} selected</Text>
              </View>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {ALL_TAGS.map(tag => (
                  <TagChip key={tag} tag={tag} selected={form.tags.includes(tag)} onClick={() => toggleTag(tag)} accentColor={C.blue} />
                ))}
              </View>
            </View>
        ...
        {step === 3 && (
          <View>
            ...
            <View style={{ backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.border, padding: 12, marginBottom: 10 }}>
              ...
              <TouchableOpacity onPress={() => setShowLocationPicker(true)} activeOpacity={0.8} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10, borderWidth: 1.5, borderColor: form.address ? C.blue : C.border, backgroundColor: C.surface2, marginBottom: 10 }}>
                <MaterialCommunityIcons name="map-marker-outline" size={18} color={form.address ? C.blue : C.textMid} />
              ...
              <Input label="Pickup instructions (optional)" value={form.pickup_instructions} onChangeText={set('pickup_instructions')} placeholder="e.g. Ring bell, call before coming" accentColor={C.blue} />
            </View>
            ...
            <View style={{ backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.border, padding: 12 }}>
              ...
              {[
                ...
                <TouchableOpacity key={key} onPress={open} activeOpacity={0.8} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10, borderWidth: 1.5, borderColor: (form as any)[key] ? C.blue : C.border, backgroundColor: C.surface2, marginBottom: i === 0 ? 8 : 0 }}>
                  <MaterialCommunityIcons name={icon} size={16} color={(form as any)[key] ? C.blue : C.textMid} />
                  <View style={{ flex: 1 }}>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.border, padding: 16, paddingBottom: 16 + insets.bottom }}>
        <Btn fullWidth size="lg" variant="blue" onPress={step < 3 ? next : submit} disabled={loading} loading={step === 3 && loading}>
          ...
          <View style={{ backgroundColor: C.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: insets.bottom + 8 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: C.border }}>
              ...
              <TouchableOpacity onPress={() => {
                ...
                <Text style={{ fontSize: 15, color: C.blue, fontWeight: '700' }}>Done</Text>
              </TouchableOpacity>
            </View>
            <DateTimePicker
              value={tempDate}
